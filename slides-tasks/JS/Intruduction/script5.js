...
function fortuneTeller(name) {
  ...
  numberOfChildren =
    numberOfChildernOptions[randomInt(0, numberOfChildernOptions.length)];
  partnerName = partnerNameOptions[randomInt(0, partnerNameOptions.length)];
  geographicLocation =
    geographicLocationOptions[randomInt(0, geographicLocationOptions.length)];
...
function dateDiff() {
  ...
  document.getElementById(
    "dateDiff"
  ).innerText = `${diff} days between those two dates.`;
}